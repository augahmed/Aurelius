...
- when the active backend is GPT-2, the server applies conservative web-specific limits so chat stays responsive on the current non-cached CPU path: short reply caps, recent-history trimming, per-message prompt trimming, and cache disabled
